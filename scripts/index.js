...
searchbarElement.addEventListener('keydown', (event) => {
  if (
    (searchbarElement.value === 'happy Krieger' &&
      event.key === 'Enter' &&
      searchTab === null) ||
    searchTab.closed
  ) {
    searchTab = window.open('the-happy-warior-file.html', '_blank');
  }
  if (
    (searchbarElement.value === 'happy Krieger' &&
      event.key === 'Enter' &&
...
function howareu() {
  ...
  document.querySelector('.evil-felix').innerHTML =
    `<a href="EvilFelix.html" class="evil-site">./...-/../.-..//..-././.-../../-..-</a>`;
}
function gut() {
